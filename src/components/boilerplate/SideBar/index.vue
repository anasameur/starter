        ...
        <template v-for="(item, i) in routes">
          ...
          <v-list-group v-if="item.children" :key="i" :group="item.meta.group" :prepend-icon="item.meta.icon" no-action="no-action">
            <v-list-tile slot="activator" ripple="ripple">
              <v-list-tile-content>
                <v-list-tile-title>{{ generateRouteTitle(item.meta.title) }}</v-list-tile-title>
              </v-list-tile-content>
            </v-list-tile>
            <template v-for="(subItem, j) in item.children">
              ...
              <v-list-group v-if="subItem.children" :key="j" :group="subItem.meta.group" no-action="no-action">
                <v-list-tile slot="activator" ripple="ripple">
                  ...
                  <v-list-tile-content>
                    <v-list-tile-title>{{ generateRouteTitle(subItem.meta.title) }}</v-list-tile-title>
                  </v-list-tile-content>
                </v-list-tile>
                <v-list-tile v-for="(subSubItem, k) in subItem.children" :key="k" :to="{ name: subSubItem.name }" ripple="ripple">
                  <v-list-tile-action>
                    <v-icon>{{ subSubItem.meta.icon }}</v-icon>
                  </v-list-tile-action>
                  <v-list-tile-content>
                    <v-list-tile-title>{{ generateRouteTitle(subSubItem.meta.title) }}</v-list-tile-title>
                  </v-list-tile-content>
                </v-list-tile>
              </v-list-group>
              <!--child item-->
              <v-list-tile v-else :key="j" :to="{ name: subItem.name }" ripple="ripple">
                <v-list-tile-action>
                  <v-icon>{{ subItem.meta.icon }}</v-icon>
                </v-list-tile-action>
                <v-list-tile-content>
          ...
          <!--top-level link-->
          <v-list-tile v-else :to="{ name: item.name }" ripple="ripple" :key="i">
            <v-list-tile-action>
